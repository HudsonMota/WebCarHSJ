						...
						<div class="form-group col-md-12">
							...
							<div class="ls-custom-select">
								<select id="setorsolicitante[0]" name="setorsolicitante" class="ls-select form-control" required>
									...
									@foreach($sectors->getSectors()->sortBy('sector') as $sectors)
									<option value="{{$sectors->cc}}" @if($vehiclerequest->setorsolicitante == $sectors->cc) selected @else @endif class=" form-control">{{$sectors->sector}}</option>
									@endforeach
								</select>
							</div>
						</div>
